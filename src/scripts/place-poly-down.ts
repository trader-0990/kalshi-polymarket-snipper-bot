 ...
 * Bitcoin 15m up/down Polymarket.
 *
 * Usage: npx ts-node src/scripts/place-poly-down.ts [price] [size]
 * Requires: POLYMARKET_PRIVATE_KEY, POLYMARKET_PROXY in .env.
 */
import "dotenv/config";
import { getPolymarketAskPrices, getTokenIdsForSlugCached } from "../polymarket/prices";
import { placePolymarketOrder } from "../polymarket/order";
import { ARB_POLY_MIN, ARB_SIZE, POLYMARKET_MIN_USD } from "../core/config";
...
async function main(): Promise<void> {
  ...
  const { downTokenId, conditionId } = await getTokenIdsForSlugCached(slug);
